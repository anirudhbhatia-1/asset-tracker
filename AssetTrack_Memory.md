...
2026-07-28 | Antigravity AI | Responsive Layout Fixes | ✅ Conducted full responsive audit of the app. Fixed breakage points: Replaced fixed grid columns (e.g. `grid-cols-4` -> `grid-cols-1 sm:grid-cols-2 lg:grid-cols-4`) in modals and cards, added `viewBox` for fluid scaling of the SVG donut chart, added flex wrapping for text overflow, and replaced fixed widths with fluid max-widths in Scanner views.
2026-07-28 | Antigravity AI | Comprehensive Responsive Layout Audit & Fixes | ✅ Conducted full responsive analysis of all 12 pages across 320px→1920px. Fixed: Modal.jsx body scroll clipping, Tickets/Onboarding w-max filter bar overflow, AssetDetail sticky sidebar at tablet breakpoint (md:grid-cols-[2fr_1fr] + md:sticky), Inventory header flex-wrap, FilterToolbar select min-widths, InventoryBreakdown legend truncation, Employees dept filter horizontal scroll fade, Login card responsive padding, Settings tab spacing. Build verified clean (✓ 249ms). Pushed to GitHub.
2026-07-28 | Antigravity AI | Layout Cleanup & User Menu Implementation | ✅ Removed redundant "NAVIGATION" label from Sidebar and page-titles from TopBar after confirming each page has its own `<h1>`. Audited `TopBar` and confirmed it's a shared component across all roles (Admin, Employee, HR) in `MainLayout.jsx`. Replaced standalone TopBar user info with a headless UI User Menu dropdown. Implemented a secure `ChangePasswordModal` allowing users to change their passwords. Created a `PATCH /change-password` endpoint that verifies current passwords, updates bcrypt hashes, and aggressively invalidates all active sessions to force a re-login. Updated Design and Rules documentation.
2026-07-29 | Antigravity AI | History Logs Actor Identity Fix | ✅ Found root cause of hardcoded 'Rajan Sharma' fallback in assetService methods. Updated `asset_history` schema to include `actor_id` (foreign key to employees), refactored route handlers to pass authenticated `req.user` to services, and modified `historyService` to capture actual user identity on write and join display names on read. Updated Engineering Rules to enforce dynamic session-based logging.
2026-07-30 | Antigravity AI | Ticket Lifecycle & Dashboards Fixes | ✅ Implemented a robust `confirm-close` and `reopen` API for resolved tickets. Updated Employee Dashboards with readable ticket IDs, terminal states, resolution notes, and wired Confirm/Reopen buttons. Enhanced HR Dashboard with 3 key metric cards, onboarding filter chips, and an items summary view. Fixed height/scrolling layout discrepancies across both dashboards. Added a data cleanup script for dummy assets.
2026-07-31 | Antigravity AI | Dynamic Office Locations Fixes | ✅ Replaced hardcoded OFFICE_LOCATIONS array with dynamic DB-driven locations everywhere. Added Locations table and full settings UI for admins to add locations and HR to link addresses. Fixed forms to use dynamic dropdowns. Updated onboarding payload mapping.
2026-07-31 | Antigravity AI | UI Refinements & Data Fixes | ✅ Diagnosed a mismatch between DB roles and UI state caused by admins testing the "Grant Login Access" flow with personal emails. Implemented an email domain validation safeguard (only `@company.com` or `@thinkvibes.com`) for login provisioning. Cleaned up multiple layout issues including fixing `EmployeeCard` location display (reverting to short location string and adding full address in tooltip), fixing card footer alignment with fallbacks, removing fixed width bounds causing premature search bar / admin email truncation, correctly sizing department pills, adding title-casing to display names, explicitly badging system service accounts, and polishing the `Employees` filter area for accessibility and clarity.
2026-08-03 | Antigravity AI | Dashboard Inventory Overview Enhancement | ✅ Extended `InventoryBreakdown` widget with 4 grouping dimensions (Category, Location, Status, Warranty) returning unified `{ label, count, percentage, color }` shapes. Added interactive hover tooltips, connected chart/legend hover-sync, and click-through navigation to pre-filtered Inventory views (`/inventory?filterKey=filterValue`). Note: Test-category cleanup remains an open, deferred item.
2026-08-03 | Antigravity AI | Absolute Timestamp Formatting & Actor Truncation Fix | ✅ Replaced relative time strings with unified `formatDateTime` (`dd/MM/yyyy, h:mm a`) across `TicketDetailsModal.jsx`, `HistoryTimeline.jsx`, `ActivityFeed.jsx`, and `NotificationPanel.jsx`. Root cause of "Admi" truncation bug: Employee ID 14's `name` column was saved as `'Admi'` in the DB `employees` table; fixed DB row to `'Admin'` and updated legacy history entries. Files touched: `client/src/utils/formatters.js`, `client/src/components/tickets/TicketDetailsModal.jsx`, `client/src/components/asset-detail/HistoryTimeline.jsx`, `client/src/components/dashboard/ActivityFeed.jsx`, `client/src/components/layout/NotificationPanel.jsx`, `AssetTrack_Rules.md`, `AssetTrack_Memory.md`.
2026-08-03 | Antigravity AI | Warranty Days Countdown, Bulk Asset Assignment & Parent-Child Sub-Assets | ✅ Implemented `getWarrantyDaysLeft` countdown utility and displayed color-coded days remaining under Warranty Expiry in `SpecsProfile.jsx`. Added `parent_id` foreign key migration (`20260804000000_asset_parent_child.sql`) to link accessories (adaptors/chargers/bags) to primary devices (laptops) with transactional cascading assignment/return logic. Added `bulkAssignAssets` service and `POST /api/employees/:id/assign-assets` route, built `BulkAssignModal.jsx` with category grouping and search filter, and added "Assign Hardware" buttons to employee cards/rows. Added `LinkedAccessoriesCard.jsx` and disabled direct assign/return actions for sub-assets.
2026-08-03 | Antigravity AI | Asset QR Tag Generator & Printable Label Sticker | ✅ Installed `qrcode.react` package in client. Created `QrTagModal.jsx` component encoding serial numbers as SVG QR codes with printable sticker preview. Added "QR Tag" button to `SpecsProfile.jsx` header. Integrated window.print popup scoped strictly to the asset tag layout. Zero backend changes required since existing scanner resolves serial numbers directly.
2026-08-03 | Antigravity AI | Excel Import & Export + Extended Asset Schema | ✅ Added migration `20260804000001_asset_extended_fields.sql` with 17 new columns. Updated `mapAsset()`, `createAsset()`, and `updateAsset()` in `assetService.js`. Created `importExportService.js` with `exceljs` multi-sheet export and transactional import (Excel date conversion, employee lookup, sub-asset adaptor auto-linking, serial deduplication). Created `GET /api/assets/export` and `POST /api/assets/import` routes with `multer`. Added Import/Export buttons to `Inventory.jsx`.
2026-08-03 | Antigravity AI | Dynamic Category-Specific Fields for Add Asset Form | ✅ Added 17 state variables and payload mapping to `AddAssetForm.jsx`. Implemented dynamic Category (Laptop vs Headphone vs Keyboard/Mouse) and Ownership Type (Company vs Client) conditional rendering logic for Extended Specifications inputs.
2026-08-04 | Antigravity AI | Role Management Tab & Role Saving Fixes | ✅ Replaced undefined `bg-info-blue/10` CSS class in `RoleManagementPanel.jsx` with standard Tailwind classes (`bg-blue-500/10 text-blue-500 border-blue-500/30`). Added explicit `hasLogin` evaluation to `mapEmployee()` in `employeeService.js` handling `has_login`, `password_hash`, `google_id`, or `role`. Updated `updateEmployeeRole` service function to SELECT back full mapped employee object. Refactored `changeRole`, `grantAccess`, and `grantGoogleAccess` hooks in `useEmployees.js` to execute `await fetchEmployeesData()` full state refreshes to eliminate stale partial employee updates.
