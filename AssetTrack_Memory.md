...
2026-07-30 | Antigravity AI | HR Onboarding Request Fixes | ✅ Addressed multiple bugs in the HR Dashboard's New Hire flow. 1) Department dropdown was empty because HR roles were denied access to `GET /api/employees`. Created a dedicated `GET /api/employees/departments` endpoint. 2) The "Edit Request" button did nothing visually or programmatically. Created `PUT /api/onboarding/:id` and built an `EditOnboardingModal.jsx` to pre-fill and save changes. 3) Fixed filter chips logic where 'arranged' status was incorrectly grouped under 'Completed' instead of 'Pending'. Fixed contrast bug on the "All" chip label by updating `text-base-100` to `text-white`. 4) Confirmed the "Hardware Requirements" row already correctly leverages dynamic category dropdowns.
